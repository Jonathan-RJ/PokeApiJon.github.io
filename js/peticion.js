...
let ApiPokemonUrl = "https://pokeapi.co/api/v2/pokemon/";
...
async function apiPokemon(url) {
    ...
    try {
        ...
        for(let i = 0; i < json.results.length; i++){
            ...
            try {
                ...
                <figure>
                    ...
                    <img class="img-pokemon" src="${pokemon.sprites.front_default}" alt="${pokemon.name}">
                    </div>
                    <figcaption>${pokemon.name}</figcaption>
                    <p>Experiencia: <span>${pokemon.base_experience}</span></p>
                    <p>Habilidadades: <br> <span>${pokemon.abilities[0].ability.name}</span></p>
                    <p>Altura: <br> <span>${pokemon.height}</span></p>
                ...
                $template += `
                <figure>
                    <figcaption>Error: ${error.status}: ${message}</figcaption></figure>
                `
            }
        ...
        $nav.innerHTML = $prev + " ϞϞ(๑⚈ ․̫ ⚈๑)∩ "+ $next; 
...
d.addEventListener("DOMContentLoaded", (e) => apiPokemon(ApiPokemonUrl));
d.addEventListener("click", (e)=>{
    if(e.target.matches(".links a") || e.target.matches(".links-2 a")){
    e.preventDefault();
    apiPokemon(e.target.getAttribute("href"))}
})